...
from equiv_primitive.policies.vision.vec_layers import VecLinear, VecResBlock, channel_equi_vec_normalize
from equiv_primitive.policies.vision.vec_layers import VecLinNormAct as VecLNA
...
from equiv_primitive.policies.utils.misc import  EQUIV_PRIMITIVE_PATH
...
class VecDGCNN_att(nn.Module):
    def __init__(
        ...
        for i in range(self.num_layers):
            ...
            if i == 0:
                self.V_list.append(VecLNA(3, feat_dim[0], mode="so3", act_func=act_func, return_tuple=self.return_tuple))
            else:
                self.V_list.append(
                    VecLNA(feat_dim[i - 1] * 2, feat_dim[i], mode="so3", act_func=act_func, return_tuple=self.return_tuple)
                )
            # * Make Global Pooling Layers
            if use_res_global_conv and i >= self.res_global_start_layer:
                self.global_conv_list.append(
                    VecLNA(feat_dim[i] * 2, feat_dim[i], mode="so3", act_func=act_func, return_tuple=self.return_tuple)
                )
            ...
            if i >= self.atten_start_layer:
                ...
                self.Q_list.append(
                    VecLNA(feat_dim[i - 1], feat_dim[i], mode="so3", act_func=act_func, return_tuple=self.return_tuple)
                )
                self.K_list.append(
                    VecLNA(feat_dim[i - 1] * 2, feat_dim[i], mode="so3", act_func=act_func, return_tuple=self.return_tuple)
                )
    ...
    def forward(self, x):
        ...
        if self.center_pred:
            ...
            center = self.fc_center(x[..., None])[0].squeeze(-1)
...
class VecDGCNN_att_frozen(VecDGCNN_att):
    def __init__(
        self,
        h_dim=128,
        c_dim=128,
        ...
        preload_path_full = os.path.join(EQUIV_PRIMITIVE_PATH, preload_path)
...
def test_pretrained():
    ...
    w_enc_path = os.path.join(EQUIV_PRIMITIVE_PATH, "pretrained/mugs.pt")
    dataset_dir = os.path.join(EQUIV_PRIMITIVE_PATH, 'data/transfer_tape/')
    ...
    B, N = 1, 512
    random_idx = np.random.choice(input_pc.shape[0], N, replace=False)
    input_pc = input_pc[random_idx].T
    ...
    with torch.no_grad():
        center, scale, so3_feat, inv_feat = net(pcl)
        ...
        for _ in range(10):
            ...
            aug_center_hat, aug_scale_hat, aug_so3_feat_hat, aug_inv_feat_hat = net(aug_pcl)
